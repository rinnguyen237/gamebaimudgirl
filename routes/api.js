								...
								if (!!dataMG) {
									...
									UserInfo.findOneAndUpdate({'id':napthe.uid}, {$inc:{red:nhan}}, function(err2, user) {
										if (void 0 !== redT.users[napthe.uid]) {
											...
											// sdt 03943433.....
											//redT.telegram.sendMessage('1090916551', user.name + ' nạp thẻ cào thành công:'  + Helper.numberWithCommas(napthe.menhGia) + ' ', {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
											}));
										}
									});
