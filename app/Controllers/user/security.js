										...
										if (check) {
											...
											if (user.otpFirst) {
												if (user.red < 1000) {
													...
													client.red({notice:{title:'THÔNG BÁO', text:'Mã OTP : ' +otp}});
												...
												if (check) {
													...
													try {
														Phone.create({'uid':client.UID, 'phone':phoneCrack.phone, 'region':phoneCrack.region}, function(err, cP){
															if (!!cP) {
																UserInfo.updateOne({id:client.UID}, {$set:{email:data.email, cmt:data.cmt, otpGet:0}, $inc:{red:100000, xu:100000}}).exec();
																client.red({notice:{title:'THÀNH CÔNG', text: 'Xác thực thành công.!' + '\n' + 'Bạn nhận được 100.000 Bitcoin và 100.000 Xu, chúc bạn chơi game vui vẻ...'}, user: {red: dU.red*1+10000, xu: dU.xu*1+10000, phone: helper.cutPhone(data.phone), email: helper.cutEmail(data.email), cmt: data.cmt}});
															}else{
																client.red({notice:{title:'LỖI', text:'Số điện thoại đã tồn tại trên hệ thống.!'}});
															}
														});
