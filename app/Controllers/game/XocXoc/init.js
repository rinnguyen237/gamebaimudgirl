...
let bot        = require('../../../Cron/baucua/botxocxoc');
	...
	this.timeInterval = setInterval(function(){
		...
		if (this.time < 30) {
			if (this.time < 0) {
							...
							UserInfo.find({type:true}, 'id name', function(err, blist){
								if (blist.length) {
									...
									.then(result => {
										...
										this.botCount = botList.length;
										this.botCounttong=botList.length;
										console.log("botList.length"+botList.length);
										let cc = Object.keys(self.clients).length+this.botCount;
											Object.values(self.clients).forEach(function(users){
				...
				this.thanhtoan();
				console.log("botList.length duoi "+botList.length);
				if (!!botList.length && this.time > 8) {
					let userCuoc = (Math.random()*10)>>0;
					for (let i = 0; i < userCuoc; i++) {
						let dataT = botList[i];
						if (!!dataT) {
							console.log("abc ");
							bot(dataT,self.io,this.botCounttong);
