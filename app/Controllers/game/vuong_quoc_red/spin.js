		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
				if (client.VuongQuocRed.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					client.red({VuongQuocRed:{status:0, notice:'Bạn không đủ ' + (red ? 'BitCoin':'XU') + ' để quay.!!'}});
				}else{
					...
					HU.findOne({game:'vuongquocred', type:bet, red:red}, {}, function(err2, dataHu){
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											red && client.redT.sendInHome({pushnohu:{title:'VƯƠNG QUỐC BitCoin', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'VƯƠNG QUỐC BitCoin', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
