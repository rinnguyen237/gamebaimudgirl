...
var Users_xocxoc        = require('../../../../Models/XocXoc/XocXoc_user');
...
module.exports = function(client, id){
	if (!!id) {
		...
		var wait_xocxoc = new Promise((resolve, reject)=>{
			Users_xocxoc.findOne({'uid': id}, function(err, result) {
				if (!!result) {
					result = result._doc;
					delete result._id;
					delete result.uid;
		...
		var active = [wait_user, wait_TX, wait_BauCua, wait_BigBabol, wait_CaoThap, wait_Mini3Cay, wait_miniPoker, wait_VuongQuocRed, wait_AngryBirds, wait_LongLan, wait_Candy,wait_xocxoc];
