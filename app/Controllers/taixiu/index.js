	...
	if (!!str) {
		UserInfo.findOne({id:client.UID}, 'red', function(err, user){
			if (!user || user.red < 1000) {
				client.red({taixiu:{err:'Tài khoản phải có ít nhất 1.000 BitCoin để chat.!!'}});
			}else{
			...
			if (bet < 1000) {
				...
				UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
					if (user === null || (red && user.red < bet) || (!red && user.xu < bet)) {
						client.red({taixiu:{err:'Bạn không đủ ' + (red ? 'BitCoin':'Xu') + ' để cược.!!'}});
					}else{
							...
							if (!!isCuoc) {
								...
								if (isCuoc.select !== select) {
									...
									if (taixiu) {
										if (red) {											
											if (select) {
												//client.redT.telegramUseronline.sendMessage('1090916551', user.name +"Cược " + Helpers.numberWithCommas(bet) +" Đặt tài :Game Tài xỉu " , {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
												io.taixiu.taixiu.red_tai      += bet;

												io.taixiuAdmin.taixiu.red_tai += bet;
											}else{
												//client.redT.telegramUseronline.sendMessage('1090916551', user.name +"Cược " + Helpers.numberWithCommas(bet) +" Đặt Xỉu Game Tài xỉu " , {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
