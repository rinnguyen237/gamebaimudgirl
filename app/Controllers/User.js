									...
									try {
										UserInfo.create({'id':client.UID, 'name':name, 'joinedOn':new Date()}, function(errC, user){
											if (!!errC) {
												...
												Message.create({'uid': client.UID, 'title':'Thành Viên Mới', 'text':'Chào mừng bạn đến với MUDGIRL , chúc bạn chơi game vui vẻ...', 'time':new Date()});
												Message.create({'uid': client.UID, 'title':'Tặng Code game', 'text':'1:Bạn đăng kí OTP : Sẽ nhận được 100,000 Bitcoin và 10.000 Xu.  \n 2 :Bạn đã nhận được Code game từ 100 - 20k chưa. Nếu chưa thì Nhắn tin ngay cho fanpage chúng tôi để tiến hành nhận code nào. Chúc AE thăng lớn.  Thân ái', 'time':new Date()});
												Message.create({'uid': client.UID, 'title':'Khuyến mại nạp 150%', 'text':'Lần đầu nạp ,Khuyến Mại 150% giá trị nạp. Nạp ngay thôi để thắng lớn và nhận ưu đãi. Chúc Bạn thắng lớn !', 'time':new Date()});


												GameState(client);
												client.red(data);
												//client.redT.telegramUseronline.sendMessage('1090916551', name + " : Người dùng vừa tạo tài khoản mới:" , {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
