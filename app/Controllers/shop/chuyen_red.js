	...
	if (!!data && !!data.name && !!data.otp) {
		if (!validator.isLength(data.name, {min: 3, max: 17})) {
			...
			if(validator.isEmpty(name) ||
				...
				otp.length != 4)
			{
				client.red({notice:{title:'CHUYỂN BitCoin', text:'Kiểm tra lại các thông tin.!'}});
			}else{
							...
							if (data_otp && otp == data_otp.code) {
								if (((new Date()-Date.parse(data_otp.date))/1000) > 180 || data_otp.active) {
									...
									.then(valuesCheck => {
										...
										if (!!to) {
											if (to.id == client.UID) {
												client.red({notice:{title:'CHUYỂN BitCoin',text:'Bạn không thể chuyển cho chính mình.!!'}});
											}else{
												if (user == null || (user.red-10000 < red)) {
													...
													if (void 0 !== client.redT.users[to.id]) {
														Promise.all(client.redT.users[to.id].map(function(obj){
															obj.red({notice:{title:'CHUYỂN BitCoin', text:'Bạn nhận được ' + Helper.numberWithCommas(thanhTien) + ' BitCoin.' + '\n' + 'Từ người chơi: ' + client.profile.name}, user:{red: to.red*1+thanhTien}});
														}));
													}
											...
											client.red({notice:{title:'CHUYỂN BitCoin',text:'Người dùng không tồn tại.!!'}});
										}
									})
								}
							}else{
