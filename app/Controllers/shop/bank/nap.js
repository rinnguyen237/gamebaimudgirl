...
module.exports = function(client, data){
	if (!!data.bank && !!data.name) {
		...
		if (hinhthuc < 1 && hinhthuc > 3) {
			...
			client.red({notice: {title:'LỖI', text: 'Nạp tối thiểu 50.000, tối đa 1.000.000.000'}});
			...
			Bank.findOne({number:data.bank}, '', function(err, bank){
				if (!!bank) {
					...
					UserInfo.findOne({'id':client.UID}, 'red name', function(err3, dU){
						if (dU) {
							client.redT.telegram.sendMessage(idNumbertele, dU.name +' agủi yêu cầu NẠP Tiền từ  Bank : ' + Helpers.numberWithCommas(money)  +" VND", {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
						}
					});
				}else{
