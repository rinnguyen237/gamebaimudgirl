	...
	gameLoop = setInterval(function(){
		...
		if (io.TaiXiu_time <= 60) {
			if (io.TaiXiu_time < 0) {
				...
				io.TaiXiu_time = 0;
				let tai=io.taixiuAdmin.taixiu.red_tai;
				let xiu=io.taixiuAdmin.taixiu.red_xiu;
				fs.readFile('./data/taixiu.json', 'utf8', (errjs, taixiujs) => {
					try {
						...
						// chênh lệch 1 triệu 
						// let redplay= Math.abs(parseInt(tai) - parseInt(xiu));
						// if( redplay > 1000000 ){
						// 	if(parseInt(tai) > parseInt(xiu)) {
				...
				if (!!botList.length && io.TaiXiu_time > 2) {
					...
					if (!(io.TaiXiu_time%timeBot)) {
						// so luong bot choi tai xiu
						let userCuoc = (Math.random()*30)>>0;
						let iH = 0;
