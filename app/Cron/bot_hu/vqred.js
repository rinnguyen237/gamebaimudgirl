	...
	HU.findOne({game:'vuongquocred', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if(line_win.win == 5) {
					if (line_win.type === 5) {
						...
						io.sendInHome({pushnohu:{title:'VƯƠNG QUỐC BitCoin', name:user.name, bet:okHu}});
						nohu = true;
			...
			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'VƯƠNG QUỐC BitCoin', users:user.name, bet:bet_win, status:2}}});
			}
